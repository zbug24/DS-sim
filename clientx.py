...
def pick_server(servers, job_cores, job_runtime):
    ...
    for s in servers:
        ...
        if state in ("active", "idle"):
            ...
            if avail_cores >= job_cores:
                ...
                # Starting inactive servers increases rental cost.
                # Prefer cheaper inactive servers, while still considering fit.
                inactive_score = (hourly_rate * 5) + surplus

                tier3.append((inactive_score, hourly_rate, surplus, server_id, s))
    ...
    if tier3:
        tier3.sort(key=lambda x: (x[0], x[1], x[2], x[3]))
        return tier3[0][4]
